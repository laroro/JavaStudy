...
-생성된 배열은 Heap(힙) 메모리 공간에 저장되고, 할당된 메모리의 주소값을 반환한다.
-따라서 배열명(배열변수)는 주소값(참조값)을 할당받게 된다.
-우리는 해당 주소를 통해 배열에 접근한다.
 */
public class E01OneDimArray01 {
  ...
  public static void main(String[] args) {
    ...
    배열명.length : 해당 배열의 크기를 반환한다. 배열의 크기가 변경되더라도 length를 통해 접근할 수 있으므로 유지보수에 용이하다.
     */
    ...
    /*
    인스턴스배열 : 기본자료형을 저장하는 배열이 아닌 인스턴스의 참조값을 저장할 용도로 생성하는 배열을 말한다
    사용법은 일반 배열과 동일하다.
     */
    ...
    System.out.println("방법1: 배열의 선언 후 초기화");
    int[] initArr = new int[1];
    initArr[0] = 100;
    System.out.println("initArr[0]=" + initArr[0]);

    /*
    배열을 선언과 동시에 초기화하는 경우 자바 컴파일러는 초기값의 갯수를 통해 크기를 결정할 수 있으므로 크기는 생략하도록 한다.
     */
    System.out.println("방법2: 배열 선언과 동시에 초기화1");
    // int[] initArr2 = new int[3] {1, 20, 300};
    int[] initArray2 = new int[] {1, 20, 300};

    System.out.println("방법3 : 초기화값만으로 선언");
    /*
    배열 요소의 갯수를 통해 배열을 생성한 후 초기화까지 동시에 진행한다. 이 경우 new를 생략할 수 있다.
    이 경우는 new를 사용하지 않고 초기화를 진행한다.
     */
