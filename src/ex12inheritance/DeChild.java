...
public class DeChild extends DeParent {
    ...
    /*
    오버라이딩 아님 : 부모클래스에서 private 멤버메소드로 선언했으므로
    자식쪽에서는 보이지 않으므로 오버라이딩의 대상이 될 수 없다.  
    ...
    /*
    메서드 오버라이딩의 조건
    1. 부모클래스에서 선언될때와 비교하여 접근범위가 동일하거나 그보다 넓으면 가능하다.
    2. 부모클래스에서 default로 선언하면 자식클래스에서는 default 혹은 그보다 넓은
        public으로 선언하면 오버라이딩이 성립한다.
     */
    @Override
    String sleep() {
    ...
    public void printParent() {
      ...
      super.멤버메소드 : 부모클래스의 멤버변수 혹은 멤버메소드를 호출한다.
       */
        super.printParent();
        System.out.printf("학번: %s\n", stNumber);
    }
    /*
    static(정적메소드) 는 오버라이딩의 대상이 될 수 없다. 클래스
    외부(메서드영역)에 별도로 로드되므로 오버라이딩이 되지않고
    호출시 각 클래스명을 통해 호출해야 한다.
     */
