...
//고등학교 친구의 정보를 저장하기 위한 자식클래스
 class HighFriend extends Friend {
    ...
    /*
    오버라이딩1: 고딩친구의 전체정보를 출력하기ㅏ 위해 부모에서 정의한
  ...
  class UnivFriend extends Friend {
	  //생성자
    String major;
    //오버라이딩1,2역시 High클래스와 동일한 패턴으로 정의함
...
public class E10MyFriendInfoBook {
	...
	정의한다. main 메서드에서 직접 호출하기 위해 static으로 선언되어 있다.
	 */
	...
	/*
	메인메서드는 해당 프로그램의 시작점(Entry point)이므로 복잡한 로직의
  ...
  public static void main(String[] args) {
    ...
    while (true) {
      ...
      //2.스캐너 클래스를 통해 사용자로부터 수행할 기능의 메뉴를 입력받음 
...
class FriendInfoHandler {
	...
	참조할 수 있으므로 부모타입의 배열에서 자식타입의 인스턴스를 모두
	저장할 수 있다.
	 */
  ...
  /*
  배열에 저장된 연락처 정보를 카운터하기 위한 변수로, 추가할때마다
  ++(증가연산자)로 1씩 증가시킨다. 
   */
  ...
  public void addFriend(int choice) {
    ...
    System.out.print("전화: "); iPhone = scan.nextLine();
  ...
  2.이떄 인스턴스는 Friend로 자동형변환(업캐스팅)되어 저장된다.
  ...
  public void showSimpleData() {
	...
	자동으로 형변환(업캐스팅)된다. 이때 자식의 정보를 춫력하는
  ...
  public void searchInfo() {
    ...
    for(int i=0; i<numOfFriends; i++) {
    	...
    	정보를 출력한다. compareTo대신 equals를 사용해도 결과는
    	동일하다.
    	 */
  ...
  public void deleteInfo() {
    ...
    /*
    배열의 원소중 삭제된것의 인덱스 값을 저장하기 위한 변수로
